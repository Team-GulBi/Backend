...
import com.gulbi.Backend.domain.rental.product.service.CategoryService;
...
public class CategoryController {
    ...
    public ResponseEntity<RestApiResponse> getBigCategory() {
        List<CategoryDto> list = categoryService.getBigCategories();
        RestApiResponse response = new RestApiResponse(SuccessCode.CATEGORY_FETCH_SUCCESS, list);
    ...
    public ResponseEntity<RestApiResponse> getMidCategory(@PathVariable("categoryId") Integer categoryId) {
        List<CategoryDto> list = categoryService.getBelowCategoriesByParentId(categoryId);
        RestApiResponse response = new RestApiResponse(SuccessCode.CATEGORY_FETCH_SUCCESS, list);
        return ResponseEntity.ok(response);
    }
